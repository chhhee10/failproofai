---
title: Variáveis de ambiente
description: "Configure o comportamento do failproofai com variáveis de ambiente"
---

## Dashboard

| Variável | Descrição |
|----------|-----------|
| `PORT` | Porta do dashboard (padrão: `8020`) |
| `CLAUDE_PROJECTS_PATH` | Substitui o local onde as pastas de projetos do Claude Code são encontradas |
| `FAILPROOFAI_DISABLE_PAGES=policies,projects` | Páginas do dashboard separadas por vírgula para ocultar |
| `FAILPROOFAI_ALLOWED_DEV_ORIGINS` | Hosts/IPs com permissão de acesso aos recursos de desenvolvimento. Equivalente a `--allowed-origins`. |

## Logging

| Variável | Descrição |
|----------|-----------|
| `FAILPROOFAI_LOG_LEVEL=info\|warn\|error` | Nível de log do servidor (padrão: `warn`) |
| `FAILPROOFAI_HOOK_LOG_FILE` | Caminho personalizado para o arquivo de log de hooks, ou `true` para o padrão (`~/.failproofai/logs/hooks.log`) |

## Telemetria

| Variável | Descrição |
|----------|-----------|
| `FAILPROOFAI_TELEMETRY_DISABLED=1` | Desativa a telemetria anônima de uso |

## Autenticação

| Variável | Descrição |
|----------|-----------|
| `FAILPROOF_API_URL` | Substitui a URL base do servidor de API usada pelo `failproofai auth` e pelo diálogo de autenticação do dashboard. O padrão é `https://api.befailproof.ai`; defina como `http://localhost:8080` (ou outro endereço) ao executar um servidor de API local. |
| `FAILPROOFAI_AUTH_DIR` | Substitui o local onde o `auth.json` é armazenado (padrão: `~/.failproofai`). Útil principalmente para testes isolados. |

## Prompt de primeira execução

| Variável | Descrição |
|----------|-----------|
| `FAILPROOFAI_NO_FIRST_RUN=1` | Ignora o prompt que oferece a instalação de políticas na primeira invocação simples do `failproofai` |

## LLM (para avaliação de políticas)

| Variável | Descrição |
|----------|-----------|
| `FAILPROOFAI_LLM_BASE_URL` | Endpoint da API do LLM (padrão: `https://api.openai.com/v1`) |
| `FAILPROOFAI_LLM_API_KEY` | Chave de API para políticas baseadas em LLM |
| `FAILPROOFAI_LLM_MODEL` | Nome do modelo (padrão: `gpt-4o-mini`) |